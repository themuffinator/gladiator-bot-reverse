...
Clears subsystem bookkeeping so setup/shutdown order can restart cleanly.
=============
*/
...
Clears the cached library configuration variables.
=============
*/
...
Snapshots configuration values from the bridge into the local cache.
=============
*/
...
Initialises the AAS subsystem state.
=============
*/
...
Initialises the EA subsystem for the configured client limit.
=============
*/
...
Loads the weapon AI library configuration.
=============
*/
...
Initialises the AAS sound data for bot sensing.
=============
*/
...
Initialises shared utility subsystems and bridge configuration helpers.
=============
*/
...
Tears down the utility subsystem stack in reverse order.
=============
*/
...
Initialises the botlib subsystems following the HLIL setup ordering.
=============
*/
int BotSetupLibrary(void)
{
	if (g_library_initialised) {
		BotLib_Print(PRT_ERROR, "bot library already setup\n");
		return BLERR_LIBRARYALREADYSETUP;
	}

	if (g_import_table == NULL || g_import_table->BotLibVarGet == NULL) {
		BotLib_Print(PRT_ERROR, "BotSetupLibrary: invalid import table\n");
		return BLERR_INVALIDIMPORT;
	}
	...
	if (!BotMemory_Init(BOT_MEMORY_DEFAULT_HEAP_SIZE)) {
		BotLib_Print(PRT_ERROR, "BotSetupLibrary: memory allocation failed\n");
		return BLERR_INVALIDIMPORT;
	}
...
Tears down the botlib subsystems and releases shared state.
=============
*/
