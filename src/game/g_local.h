...
/* PORT(c99): declared extern here but defined `static` in g_items.c, and
   referenced from no other translation unit.  A static definition after a
   non-static declaration is a constraint violation that MSVC6 accepted and
   modern C rejects.  The definitions keep their internal linkage (the
   author's evident intent); these dead declarations are removed.  Was:
...
#ifdef ZOID
/* PORT(c99): defined `static` in p_weapon.c and used nowhere else; see the
   armor-index note above.  Was:  extern	qboolean	is_quad; */
#endif //ZOID
