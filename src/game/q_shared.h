...
/* PORT(posix): the 1999 source was built against the MSVC CRT and uses a few
   of its non-standard spellings directly.  Supply them where libc does not,
   rather than editing every call site.  Each is the exact standard
   equivalent, so behaviour is unchanged.

   min/max are deliberately the MSVC CRT's macros character for character
   (ucrt/stdlib.h), double evaluation included.  Do NOT "fix" them into inline
   functions: two call sites pass arguments with side effects --
   m_widow2_rogue.c:1358 passes random(), m_widow_rogue.c:1611 passes
   CountPlayers() -- and evaluating those once instead of twice would make the
   Linux build behave differently from the Windows build and from retail. */
#ifndef _WIN32
#include <strings.h>		/* strcasecmp, reached via -Dstricmp=strcasecmp */
#include <unistd.h>		/* access(), used by bl_main.c */
#ifndef _isnan
