...
/* PORT(types): was `qboolean`, but this is only ever stored in gitem_t::drop,
   which is `void (*)(edict_t *, gitem_t *)`.  Every other drop handler returns
   void and this one's value is never read, so returning void matches the slot
   and removes an incompatible-pointer initialisation modern C rejects. */
void CTFDrop_Flag(edict_t *ent, gitem_t *item)
{
	...
	return;	/* PORT(types): was `return false;` -- now void, see above. */
}
