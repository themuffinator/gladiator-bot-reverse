...
- `ReadWeightConfig` cache tests should prove retained configs are returned before filesystem lookup. Load a temporary filename with `bot_reloadcharacters` off, delete the backing file, then load the same caller filename again and assert pointer identity; Q3 compares against `config->filename` before `LoadSourceFile`.【F:dev_tools/Quake-III-Arena-master/code/botlib/be_ai_weight.c†L292-L433】【F:tests/ai/test_ai_weight.c】
