...
- `MergeWeightConfigs` covers the Gladiator-only two-config merge path. Regression fixtures should use `balance(...)` leaves when asserting averages because retail only mutates balance leaf weights during this helper.【F:dev_tools/gladiator.dll.bndb_hlil.txt†L42884-L42938】【F:src/botlib/ai_weight/ai_weight.c】
