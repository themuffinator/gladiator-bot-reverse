...
Goal-selector correction: the raw Gladiator path has no last-valid-area fallback, admits raw unlinked records, ignores the bridge-only `next_respawn_time` marker, scores timed items with `+20`, pushes `GFL_ITEM` only, substitutes 30 seconds only for an exactly-zero respawn value, and arms avoidance before attempting a potentially overflowing goal-stack push. Raw dropped goals in jump-pad areas are retained; the prior successor-oriented summary above is superseded on these points.
